...
func TicketNumberCounterAdd() int64 {
	...
	db := pool.GetDB()
	resultok := db.Table("ticket_number_counter").Create(&numberCounter{
		Counter:    0,
	...
	db.Table("ticket_number_counter").Where("counter_uid = ?", counterUID).First(&nc)
	...
	db.Table("ticket_number_counter").Where("counter = 0 AND id <= ? ", nc.ID).Order("id ASC").Find(&unsetCounterIDs)
	...
	for _, unsetID := range unsetCounterIDs {
		...
		previousCounter := 0
		db.Table("ticket_number_counter").Where("id < ?", unsetID.ID).Order("id DESC").First(&tmpnc)
		...
		db.Table("ticket_number_counter").Where("id = ? AND counter = 0", unsetID.ID).Update("counter", newCounter)
	}
	var ncc numberCounter
	// Get the just inserted ticket counter with the now computed value.
	db.Table("ticket_number_counter").Where("counter_uid = ?", counterUID).First(&ncc)
	fmt.Println("ht ", ncc)
	//
	return ncc.Counter
}
